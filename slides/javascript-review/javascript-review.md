...
- In this class we will include the `"use strict";` directive at the top of ever `.js` file we create
...
- You can even add variables into strings
...
- Generally just avoid ever relying on type coercion
...
class Rabbit extends Animal {
  constructor(name,age,cuteness) {
    super(name,age); // Calls the parent/superclass's constructor
    this.cuteness = cuteness; // A new property just for a Rabbit
  }

  isCute() { // A new method just for a Rabbit
    return (this.cuteness > 50); // true if cuteness is bigger than 50
  }
}
```
...
- A class that extends another class has all the properties and methods of the super class as well as anything it adds on
...
- `null` is often returned in cases when a function that returns an object needs to return an empty/not-found/non-value
...
setTimeout(myFunction,1000); // Prints "Hello, World!" after 1000 milliseconds
```
...
- We will see: "Loop 5" "Loop 4" "Loop 3" "Loop 2" "Loop 1"
...
- One really nice way to deal with functions or constructors that have many parameters is to use an object literal instead
...
- Note how this lets us name the parameters in a nice way, too
...
- Perhaps most "interestingly" they preserve the existing context when you use `this` inside the function, which can be important especially with object oriented programming - essentially with a normal anonymous function a new context (and therefore a new `this`) is created, but with arrow functions this doesn't happen
...
- The above doesn't work because the `this` inside the anonymous function actually refers to the context of the anonymous function and __not__ the class, it will most likely print `undefined`
