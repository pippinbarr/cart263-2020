...
A reminder of some core functions and behaviours of p5.
...
// updateClown
//
// Moves the clown and checks for wrapping
function updateClown() {
  // Use a function to move the clown on the screen
  handleMovement();
  // Use a function to wrap the clown on the screen
  handleWrapping();
}
...
// wrapClown()
//
// Check if the clown is off an edge of the canvas and wrap to the other side
// if it is
function handleWrapping() {
...
function displayClown() {
  ...
  push();
  // We use tint() to set a tint on the image
  tint(clown.tint);
  // We use image() to display an image at a specified position
...
function keyPressed() {
  ...
  // Reverse the clown's velocity by negating it
  clown.vx = -clown.vx;
  clown.vy = -clown.vy;
